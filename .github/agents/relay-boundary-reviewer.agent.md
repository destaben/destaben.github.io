...
1. Compare affected code with `docs/SIGNAL-RELAY.md`, `services/signal-relay/README.md`, `compose.yaml`, and the relevant FastAPI route. For Nginx or Cloudflare Tunnel behavior, compare against the published `destaben/lab-inverse-proxy` contract rather than a local edge configuration.
