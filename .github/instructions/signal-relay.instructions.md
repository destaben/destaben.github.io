...
- Keep Nginx as the sole public HTTP policy point and retain its loopback-only host binding. The CGNAT deployment uses outbound Reticulum `TCPClientInterface` transports and an outbound Cloudflare Tunnel for HTTP only; do not add a `TCPServerInterface`, expose Reticulum TCP, or publish the relay HTTP API, Home Assistant, or any other host port.
