...
```json
{
  "status": "available",
  "nodes": [
    {"alias": "Node One", "url": "tcp://node-one.example:4242", "status": "up"}
  ]
}
```

It returns `{"status":"unavailable","nodes":[]}` when Reticulum is not active or either public node mapping is missing or inconsistent. The projection contains only configured public aliases, intentionally public `tcp://host:port` URLs and `up` or `down` statuses, with at most 12 aliases. Public aliases must be unique, begin with an ASCII letter or digit, and contain only ASCII letters, digits, spaces, periods, underscores, or hyphens, with a maximum of 48 characters. Each URL must use `tcp`, have a host and explicit port, and omit credentials, paths, queries, fragments and non-global literal IP addresses. They are public names and published endpoints for configured TCP connections, not names or bindings for the LXMF destination. `up` means the bridge currently observes the corresponding `TCPClientInterface.online` state; it is not a route, path, or delivery guarantee. Configure the private JSON mapping in `SIGNAL_RELAY_PUBLIC_TCP_NODE_ALIASES`, where private TCP client interface names map to unique public aliases, and the private `SIGNAL_RELAY_PUBLIC_TCP_NODE_URLS` JSON mapping from each alias to its intentionally public URL. Both mappings must define exactly the same aliases. Never return or log interface names, addresses other than the intentionally public URLs, or other topology. Logs record each alias's initial observed connection status, later status changes, and observation failures. They contain only the public alias and status where applicable.
